...
module('Integration | Component | Ember Power Select (Accesibility)', function(hooks) {
  ...
  test('Single-select: The trigger has `role=combobox` and `aria-owns=<id-of-dropdown>`', async function(assert) {
    assert.expect(2);
  ...
  test('Multiple-select: The trigger has `role=combobox` and `aria-owns=<id-of-dropdown>`', async function(assert) {
    assert.expect(2);
  ...
  test('Single-select: The searchbox has type `search` and `aria-owns=<id-of-listbox>`', async function(assert) {
    assert.expect(2);
    ...
    assert.dom('.ember-power-select-search-input').hasAttribute('aria-owns', /^ember-power-select-options-ember\d+$/, 'The `aria-owns` points to the id of the listbox');
  });

  test('Multiple-select: The searchbox has type `search` and `aria-owns=<id-of-listbox>`', async function(assert) {
    assert.expect(2);
    ...
    assert.dom('.ember-power-select-trigger-multiple-input').hasAttribute('aria-owns', /^ember-power-select-options-ember\d+$/, 'The `aria-owns` points to the id of the listbox');
  });

  test('Multiple-select: The listbox has `aria-multiselectable`', async function(assert) {
    assert.expect(1);
    ...
    await clickTrigger();
    assert.dom('.ember-power-select-options').hasAttribute('aria-multiselectable', 'true', 'The listbox controls the trigger');
  });
  ...
  test('Trigger can have a custom aria-role passing triggerRole', async function(assert) {
    ...
    assert.dom('.ember-power-select-trigger').hasAttribute('role', 'combobox', 'The `role` was defaults to `button`.');
  });
});
